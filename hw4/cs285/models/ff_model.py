...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction = self.forward(obs, acs,\
        data_statistics['obs_mean'], data_statistics['obs_std'], data_statistics['acs_mean'], data_statistics['acs_std'],\
        data_statistics['delta_mean'], data_statistics['delta_std'])[0]# TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        inp = self.forward(observations, actions,\
        data_statistics['obs_mean'], data_statistics['obs_std'], data_statistics['acs_mean'], data_statistics['acs_std'],\
        data_statistics['delta_mean'], data_statistics['delta_std'])[1]
        loss = self.loss(inp, target)# TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
